  ...
  return (
    <div className="flex flex-col items-center justify-center mt-10">
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 125" version="1.1"
            className="h-20 w-20 [.light_&]:fill-default-300 [.dark_&]:fill-default-600">
      ...
      <p className="[.light_&]:text-default-700 [.dark_&]:text-default-100
                    truncate overflow-hidden max-w-full sm:max-w-xl px-4">
...
function computeMicroHash(selectedFields: number[]) {
  return selectedFields.map(n => 2**n).reduce((s: number, n: number) => s + n);
}
...
function updateMeiliSearchParams(
  ...
  const selectedMeiliFields = selectedFields.map((n: number) => allMeiliFields[n]).flat();
